...
# PATH_TO_DATA_FOLDER = "./dataset/bedroom_lviv/4"
# PATH_TO_DATA_FOLDER = "./experiments/another_room_detection/hard/a_walking_in_another_room"
...
class UDP_listener():
    ...
    def get_csi_raw_data(self, csi_inf):
        ...
        # antenna_pairs = [(0, 0), (0, 1), (1, 0), (1, 1)]  # for 2tx <-> 2rx antennas
        # antenna_pairs = [(0, 0)]  # for 1 antenna
        # antenna_pairs = [(0, 0), (1, 0)]  # for 2tx <-> 1rx antenns
    ...
    def make_photo_and_save():
        ...
        if UDP_listener.packet_counter % 10 == 0:
            ret, frame = cam.read()
    ...
    def save_csi_to_file(raw_peak_amplitudes, raw_phases, carriers):
        ...
        path_to_csv_file = "{}/{}".format(PATH_TO_DATA_FOLDER, FILENAME)
        ...
        with open(path_to_csv_file, 'a', newline='\n') as csvfile:
            ...
            to_write = [*carriers]
            for amp_part in raw_peak_amplitudes:
                for num in amp_part:
                    to_write.append(num)
            for phase_part in raw_phases:
                for num in phase_part:
                    to_write.append(num)

            print("write length: ", len(to_write))
